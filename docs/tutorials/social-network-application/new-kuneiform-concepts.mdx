...
- How to interact with your database via the `kwil-js` SDK.
...
## Schema Overview
The schema contains five tables: `users`, `posts`, `likes`, `followers`, and `balances`.

The schema contains 15 actions that could be called within a social media application to execute some database CRUD operations: `create_user`, `update_user`, `add_balance`, `check_balance`, `read_post`, `create_post`, `delete_post`, `like_post`, `unlike_post`, `follow`, `unfollow`, `get_user_by_username`, `get_users_by_wallet`, `get_feed`, and `get_celebrity_feed`.

## Concepts

### View Actions

If an action returns data, it should be given a `view` access tag. The advantage to using a [`view` action](/docs/concepts#view-actions) is that it returns data directly from the node, rather than having to wait for the transaction to be mined. Any data returned from a `view` action has been mined on the network.
...
action check_balance() private view mustsign {
    SELECT
        CASE
...
action read_post($post_id) public view mustsign {
    check_balance();
