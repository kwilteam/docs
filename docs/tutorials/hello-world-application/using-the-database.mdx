...
We will also declare the database ID (DBID) and and create the `KwilSigner` to sign transactions. A DBID is a unique identifier for a database generated from the databse owner's public key and database name.
