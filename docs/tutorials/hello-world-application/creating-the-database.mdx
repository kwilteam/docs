...
Next, add the network details in the popup Menu. You can find the `chain_id` in under `~/usr/.kwild/abci/config/genesis.json`, or you can leave the Chain ID empty and the IDE will retrieve it from the local Kwil node.
