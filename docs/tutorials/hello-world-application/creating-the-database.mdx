...
This tutorial uses a local Kwil daemon, running on `http://localhost:8080`. To start a local Kwil daemon, download the latest `kwild_...` binary from the [release repo](https://github.com/kwilteam/binary-releases/releases) and run the KwilD [quickstart command](../../daemon/kwild#quickstart).

This tutorial assumes you have a basic understanding of SQL.

## Setup

The quickest way to deploy your database is in the [Kuneiform IDE](https://ide.kwil.com).
...
Next, add the network details in the popup Menu. You can find the `chain_id` in under `{root_dir}/.kwild/abci/config/genesis.json`, or you can leave the Chain ID empty and the IDE will retrieve it from the local Kwil node.

After you click "OK", the IDE will automatically switch to the new network.
...
In our Kuneiform file, add a table called `greetings` with three columns: `id`, `message`, and `wallet`:
...
Now that we have created our Kuneiform schema, click on the `Compile and Deploy` icon in left-hand navigation panel in the IDE, 
