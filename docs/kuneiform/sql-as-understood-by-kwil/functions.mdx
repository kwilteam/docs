...
Remove the longest string containing only characters in characters from the start and end of string.
...
the `from` parameter in Kwil, use `substring(string, 1, for)`.
