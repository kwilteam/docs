...
Below, we have an action that calls a procedure to get a user's name and age, and then updates the user's age.

```typescript
procedure do_something() public {
    // do something
}

action update_user_age($id uuid) public {
    do_something($id);
    update_user($username, $age + 1);
}
```

Actions cannot directly call built-in functions, and can only call them in a SQL statement.