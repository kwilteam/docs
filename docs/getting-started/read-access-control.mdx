...
To manually authenticate a user, call the `authenticate` method. In browser, the cookie will automatically be stored in the browser's cookie jar. In NodeJS, the cookie will be returned as a string and must be passed with all subsequent requests.

```typescript
async function manualAuthenticate() {
    await kwil.auth.authenticate();
}
```

### NodeJS: Manually Pass Cookie

In NodeJS, the cookie returned from the `authenticate` method must be passed with all subsequent requests.
...
async function authenticatedAction() {
    const auth = await kwil.auth.authenticate();
