...
KGW cookies are partitioned by signer and origin. This means that multiple users can be authenticated at the same time, and users authenticated from one origin (e.g., `https://website1.com`) will not be authenticated on another origin (e.g., `https://website2.com`).
