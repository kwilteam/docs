...
Kwil utilizes environment variables for configuration.  All environment variables should be prepended with `KWILD_`.  For example, to use a private key,
it should be set as the `KWILD_PRIVATE_KEY` variable.  The following environment variables are available:
