...
This guide provides step-by-step instructions for configuring and launching a single instance of a kwild node. It covers topics on 
-  How to initialize the node as a genesis node and 
-  How to join an existing network of kwild nodes during startup.

## Root directory
This is the default directory to store the all the configuration and data such as blockchain data, sql database data, signed messages etc, unless otherwise specified.
```
home_dir/
   |- config.toml
   |- private_key         
   ...
   |   |- data/ 
```
Default root directory is `~/.kwild`. This configuration can be overwritten either setting `KWILD_ROOT_DIR` environment variable or by using `--root_dir` command line flag.
...
There are multiple ways to generate this config and run the kwild node.
1. Quickstart node using --autogen flag
2. Use admin cli utils to initialize the root directory 
3. Manual configuration
This config can be automatically generated if node is run with `kwild --autogen`

### Quickstart the node using the autogen flag

Use the --autogen flag to swiftly initiate the node without delving into the configuration details. This will automatically generate any missing `private_key` and `genesis.json` files that are required for running a kwild node.

You also have the option to modify some default settings of the genesis file using below flags:
- `--without_gas_costs`
- `--without_nonces`
...
Generate private key file using the kwil-admin utils as shown below.
```bash
...
The current node can be added as one of the inital validators in the genesis file as below. The base64 public key is used here. 
```bash
  "validators": [
    {
...
```
#### Config.toml 
Download the sample [config.toml](./../configuration/default_config.toml) into `root_dir/config.toml` and update the configuration as required. The minial configuration required is: 
...
Chain configuration:
  ...
  # This can be empty for a genesis node, for others its of format "nodeID1@ip1:port1,nodeID2@ip2:port2..."
```
Refer [config.toml specification](./../configuration/configuration.mdx) for more details on configuring config.toml


## Running the genesis node
Running a kwild node successfully requires the private_key and genesis.json files. If any of these files are not configured, running kwild with the `--autogen` flag will autogenerate any missing configuration files and starts the node using the generated config:
```bash
...
## Running a non-genesis node

To start a kwild node and connect to the existing network:
...
3. Update the p2p.persistent_peers with the peer address set either in the config.toml or through the cmdline flag or through env variable and start the node.
