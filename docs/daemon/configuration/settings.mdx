...
- `tls_key_file`: This variable specifies the file path containing the corresponding private key that enables TLS on the gRPC server. Like the tls_cert_file, this path can be either absolute or relative to the determined root_dir. If unspecified, the gRPC server's communications remain unencrypted.
