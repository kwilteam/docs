...
Valid time units are "s", "m", "h". The minimum value is `1m` and the maximum value is `168h` (1 week).
...
To claim tokens from the escrow smart contract, call the `claim` action on the escrow smart contract with the inclusion proof.
