...
:::info
Right now, the Kwil team still manually deploys each bridge contract. We are working on automation to make this process easier.
In the meantime, please [reach out to us](<mailto:brennan@kwil.com>) if you would like us to deploy a bridge contract for you.
:::
...
_While the system is still manual, the Kwil team will perform this step on your behalf. This is simply here to illustrate the full deployment scenario._

The first step is to deploy the escrow contract and the relayer. The escrow contract is responsible for holding tokens, and releasing them in batch
...
relayer itself is not part of the multisig, and is merely responsible for expending ETH gas to submit transactions.**
