...
- Charging users ERC-20 tokens (or implementing gas fees) for using your application
- Implementing staking and/or validator weights in your decentralized database
...
Token distribution logic is written using [SQL Smart Contracts](/docs/language/introduction). When a user calls the `erc20.issue` method, (TODO - ADD LINK TO DESCRIPTION) the Kwil network collects the tokens and their recipients to be bridged back to the EVM chain. Rewards are collected during a 10-minute epoch. The total distribution for each epoch is signed by the Kwil Nodes with SAFE signers and relayed back to the escrow smart contract.
