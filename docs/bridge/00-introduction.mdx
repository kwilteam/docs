...
Token distribution logic is written using [SQL Smart Contracts](/docs/language/introduction). When a user calls the `erc20.issue` method, (TODO - ADD LINK TO DESCRIPTION) the Kwil network collects the tokens and their recipients to be bridged back to the EVM chain.
