...
```javascript
await kwil.call(body, kwilSigner)
```
...
Kwil Gateway is an optional service on Kwil networks that allows for authenticating users with their signatures for read queries / view procedures. Learn more about the Kwil Gateway [here](/docs/getting-started/read-access-control#kwil-gateway).
