...
If the view procedure uses a `@caller` [contextual variable](/docs/language/introduction#contextual-variables), you should also pass the `kwilSigner` to the `kwil.call()` method. This will allow the view procedure to access the caller's account identifier. The user does not need to sign for view procedures.
