...
If the view action uses a `@caller` [contextual variable](/docs/kuneiform/dml#using-contextual-variables-in-actions), you should also pass the `kwilSigner` to the `kwil.call()` method. This will allow the view action to access the caller's account identifier. Note that the user does not need to sign for view actions.
