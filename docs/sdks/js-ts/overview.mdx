...
The `WebKwil` and `NodeKwil` constructors take a configuration object. The only required fields are `kwilProvider` and `chainId`; however, you have additional configuration options available to you:
...
To verify you have the correct chainId, you can use the [`chainInfo()`](#chain-info) method.
...
In Kwil, databases are identified by a 'database identifier', which is a hex encoded SHA224 Hash of the public key and database name, prepended with an `x`.
...
Returns all database names belonging to a public key.
...
const dbid = Utils.generateDBID("public_key", "database_name");
