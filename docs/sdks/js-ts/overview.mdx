...
If the view procedure uses a `@caller` [contextual variable](/docs/kuneiform/introduction#contextual-variables), you should also pass the `kwilSigner` to the `kwil.call()` method. This will allow the view procedure to access the caller's account identifier. The user does not need to sign for view procedures.
...
Refer to the Private Mode documentation [here.](/docs/node/private-networks)
...
// pass actionBody AND kwilSigner if in Private Mode
const res = await kwil.call(actionBody, kwilSigner);
```

### Kwil Gateway Authentication

Kwil Gateway is an optional service on Kwil networks that allows for authenticating users with their signatures for read queries / view procedures. If your Kwil network used a Kwil Gateway, you should pass a `KwilSigner` to the `kwil.call()` method. If the user is not authenticated, the user will be prompted to sign a message to authenticate, and the SDK will automatically include the authentication cookie in each subsequent request.
...
To log out of the Kwil Gateway, you can call the `kwil.auth.logout()` method. This is useful if you want to switch accounts or remove the authentication cookie.

```javascript
await kwil.auth.logout();
```

For advanced authentication usage, see [read access control advanced usage](/docs/getting-started/read-access-control#advanced-usage).