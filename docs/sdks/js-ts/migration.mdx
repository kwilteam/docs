...
description: Migrate from the old Kwil SDK to the new @kwilteam/kwil-js SDK.
slug: /sdks/js-ts/migration
---
...
In the old version, accounts were identified by Ethereum Wallets. Now, they are identified by public keys (Secp256k1 or ED25519). Public keys can be represented by a Hex string or bytes (Uint8Array).
