...
The `ActionInput` class is a utility class to build inputs to be passed and executed in a procedure transaction. It is found in the `Utils` namespace, which can be imported from the `kwil` package.
