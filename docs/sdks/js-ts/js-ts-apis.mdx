...
-   `actionBody`: The message to call. This object should match the `ActionBody` interface and contain the action name and database ID. It can also contain an array of `ActionInput`s and a custom signature message, if the action requires a signature.

-   `kwilSigner`: The signer for the action transaction. This can be created with the [`KwilSigner`](#kwilsigner) class. A KwilSigner is only required if the action requires a signature.
...
Note that the `.deploy()` method not the recommended way to deploy databases on Kwil. We strongly recommend using the [Kuneiform IDE](/docs/kuneiform/introduction).
...
A promise that resolves to an `Account` object. The account object includes the owner's public key, balance, and nonce.
...
```typescript
constructor(signer: EthSigner, publicKey: HexString | Uint8Array): KwilSigner;
...
-   `publicKey`: The public key of the user that is executing the action transaction. This can be a hex string or a Uint8Array.
