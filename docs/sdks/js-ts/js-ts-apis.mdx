...
Calls a `view` procedure on the network. If authentication for the view procedure is enforced by a Kwil Gateway, or if the Kwil network is running in Private mode, a `kwilSigner` must be provided. If authentication is enforced by the Kwil Gateway, the call method will prompt the user to authenticate if they have not already done so, and will retain a cookie to authenticate with the Kwil Gateway for all subsequent calls.
...
A promise that resolves an `AuthBody` object, consisting of a signature and a challenge. Both a signature and a challenge is needed to authenticate a user in Private mode.
