...
Each account has a public_key, nonce, and balance.
...
In Kwil, databases refer to SQL deployments on a Kwil Network. The Kwil [SQL engine](/docs/category/sql-as-understood-by-kwil) is based on SQLite with modifications to support deterministic execution and consensus.

### Kuneiform

[Kuneiform](/docs/category/kuneiform) is the syntax language for defining a database on Kwil. You can think of Kuneiform as a smart contract language (e.g. [Solidity](https://soliditylang.org/)), but for relational databases.

In a Kuneiform file, you can define:
- Relational Tables
- Actions - CRUD Operations that can be executed on the database.
- Extensions - Custom logic that can be executed on the database.

### Database Identifiers

Database identifiers (DBIDs) are unique identifiers for a database on a [Kwil Network](#kwil-network). DBIDs are generated from the public key of the account that deployed the database and the database name. A DBID is a SHA224 hash of the database owner's account public key and database name.
...
View actions are read-only actions that can be executed on a database. View actions are called from a single node and may or may not require signature. The advantage of view actions is that they can be executed without the need to wait for consensus.
...
Kwil Nodes run the [Kwil Daemon](/docs/category/kwil-daemon) and are the infrastructure that power Kwil Networks. Kwil Nodes are responsible for:
