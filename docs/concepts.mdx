...
In Kwil, databases refer to relational database schemas on a Kwil Network. The Kwil [SQL engine](/docs/category/sql-as-understood-by-kwil) is based on Postgres with modifications to support deterministic execution and consensus.
...
- Actions - CRUD Operations that can be executed on the database, and call [extensions](/docs/kuneiform/advanced-features#extensions-precompiles).
