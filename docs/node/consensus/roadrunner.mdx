...
Below are the detailed info on what is being exchanged between the nodes to facilitates the consensus process during a consensus round or that triggers state transitions on different nodes of the Kwil network.
...
Validators include their signatures with the vote allowing the leader and other validators to verify the vote's authenticity and integrity.
The signed message format for validator votes depending on the Ack status is as follows:
- NACK: `BlockID + NACK`
