...
To verify that the new network is syncing changes from the old network, you can use the `kwil-admin migrate network-status` command. When used on the old node,
