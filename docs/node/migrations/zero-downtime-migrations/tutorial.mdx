...
title: Zero Downtime Migrations Tutorial
...
:::warning
The `genesis.json` and `snapshot.sql.gz` files are generated in the directory specified using the `--out-dir` flag. Ensure that you don't run the command with `--root-dir` flag set to the current node's root directory as it can overwrite the existing genesis file in the node's root directory.

If the admin service is directly accessible from the new node (e.g. it is listening on a TCP socket securely accessible from the new node) it can be run on the new node directly.
:::
...
2. [Download the snapshot and genesis files](#genesis-state) generated at the start of the migration as shown above.
    ...
    - `chain_id`: This is set based on the chainID in the migration proposal.
    ...
    - `validators`: This field is set to the validators from the existing network.
    ...
    :::warning
    `allocs` and `activations` fields are set to `null` in the genesis file. All other fields have same configurations as that on the old network. If any other changes are needed to the genesis configuration, update the genesis file manually and redistribute it to the node operators.
    :::
