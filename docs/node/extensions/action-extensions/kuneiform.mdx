...
As an example, we will be referring to a hypothetical `hello_world` extension. This extension has one method, `sayHello`, which takes a string as an argument and returns a string.
...
:::tip
The reason why `good_afteroon` and `default_hello` provide different messages is because they were initialized with different values. This was configured in the extensions original [`ExtensionInitializer`](http://localhost:3000/docs/extensions/actions/overview#example) function.
:::
