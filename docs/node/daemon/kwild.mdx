...
Notable options include the RPC and HTTP server settings such as `app.jsonrpc_listen_addr`, `chain.rpc.listen_addr` and `chain.p2p.listen_addr`.

There are two settings related to establishing connections to other network nodes:

- `chain.p2p.persistent_peers` is used to maintain persistent connections with certain nodes.
