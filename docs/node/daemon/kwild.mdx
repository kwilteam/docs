...
Configure `chain.p2p.persistent-peers` to establish and maintain persistent connections with other nodes in the network. This field can be empty if the network is made up of a standalone node. 
...
- Obtain the `genesis.json` for the desired network from another participant, and ensure that it is placed properly in your `root_dir`.
- Update the `chain.p2p.persistent_peers` with the network peer addresses and start the node. `persistent_peers` is a comma-separated string of `nodeid@ip:port` of peers to establish and maintain connections upon startup.
...
4. Note that, this node joins the network by default as a non-validator. Please refer to the [Validator Guide](/docs/node/tutorial) for more information on becoming a validator node.
