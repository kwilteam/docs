...
The `kwild` application has commands to start a blockchain node with the database services of Kwil. It also includes commands to manage the node's configuration and data.
...
This is the default directory to store all the configuration and data such as blockchain data, SQL database data, signed messages, etc, unless specified otherwise. The default root directory is `~/.kwild`. You can override this configuration by setting the `KWILD_ROOT` environment variable or using the `--root` command line flag.
