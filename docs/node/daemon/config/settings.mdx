...
## [app]   
   ...
   ### [app.snapshots]
      ...
      - `enabled`: This variable toggles the snapshot feature on or off. By default, the snapshot feature is disabled.

      - `snapshot_dir`: This variable specifies the path to the snapshots directory.

      - `snapshot_heights`: This variable specifies the heights(multiples of heights) at which the database is snapshotted. The default is 10000, which is around 1 snapshot per day considering block rate of 1 block per 6-8 secs. 
