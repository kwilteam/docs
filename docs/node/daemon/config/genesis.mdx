...
### Genesis Allocs
 `alloc`: This field specifies the Kwil accounts and their initial balances at the genesis. The `alloc` field is optional and is used to pre-fund Kwil accounts with balance. This is a JSON dictionary object where the keys are account `identifiers` of type hexadecimal bytes and the values are the account balances. The Kwil account corresponding to an ethereum account is it's address without the "0x" prefix, whereas for ed25519 accounts, it is the public key in hex format.
...
```json title="sample genesis.json file"
{
  ...
  "alloc": {
    ...
    "c89d42189f0450c2b2c3c61f58ec5d628176a1e7": 10000000000000000000000,
  }
  "consensus_params": {
    "block": {
      "max_bytes": "22020096",
