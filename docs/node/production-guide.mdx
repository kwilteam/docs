...
:::note
Since Kwil supports multiple node key types, and DB user addresses are in their own
formats that may be defined by extensions, such as checksummed Ethereum
addresses, we use this explicit public-key notation for Nodes rather than
another address node format for clarity and simplicity. This may change in the future.
:::
...
```sh
kwild key gen --key-file ./nodekey.json
```

This will write a file named `nodekey.json` in the current directory that contains the ed25519 private key for a new node. Each node **must**
...
Then copy the `nodekey.json` file created previously for this node into the root directory.
...
Finally, copy this node's `nodekey.json` file into the root directory.
