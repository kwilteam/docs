...
There are four types of extensions. Each extension has its own set of interfaces that must be implemented, as well as their
own set of rules and restrictions that must be followed.

### [Authenticators](/docs/extensions/authenticators)

Authenticators are custom asymmetric signature algorithms used to sign and verify transactions. By implementing custom authenticators,
...
Kuneiform precompiles are custom libraries that are imported and called in Kuneiform schemas. By implementing custom Kuneiform precompiles, developers
