...
This guide will walk users through the process of building a custom `kwild` binary with extensions.

## Prerequisites

Before building a custom `kwild` binary, users need to have the following installed:
- [Go 1.21+](https://golang.org/doc/install)
- [Docker](https://docs.docker.com/get-docker/)

## Step 1: Setting up the environment

Users should start by setting up the repo for building the custom `kwild` binary. This will be a standard Go ewpo setup.
...
Add the following code to `custom_extension.go`. For this example, we will add a custom authenticator extension.

:::tip
All extensions can be added in the same way, via calling their respective `Register` function
in the `init` function of the extension file.
...
To run the custom `kwild` binary, we will first have to run Postgres. The Kwil team has provided a default image
with the necessary configurations. For more information on how to run Postgres, please refer to the [Postgres setup guide](/docs/daemon/installation#postgresql).
...
After running Postgres, we can run the custom `kwild` binary. The `--autogen` flag will automatically generate a new node key and a new wallet key:
...
In this guide, we walked through the process of building a custom `kwild` binary with extensions.
Users can now use the custom `kwild` binary to run a Kwil node with the custom extension.
