...
except the app-chain logic (within the extension) should be stateless, as all state is handled by the SQL database.
...
The initialize method could then perform validation log on the passed values, add additional variables to the map, or do fire off some other process.  If an error is
returned during the `Initialize` function, it will cause the deployment of the Kuneiform schema to fail.

### Corresponding Typescript Extension:
