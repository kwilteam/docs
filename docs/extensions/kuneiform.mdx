...
see the [extensions section of the daemon documentation](../daemon/extensions.mdx), or check out the [deployment quickstart](<./quickstart.mdx>).

## Example

As an example, we will be referring to a hypothetical ERC20 extension on this page. This extension's only function is to
...
action use_caller_balance () public {
    $balance = my_token.balanceOf(address(@caller));
    // rest of the action...
}
```
...
action read_nft() public {
    $usdc_balance = usdc_token.balanceOf(address(@caller));
    $dai_balance = dai_token.balanceOf(address(@caller));
