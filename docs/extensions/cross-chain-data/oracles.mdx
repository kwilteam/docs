...
Example usecases for Kwil oracles include:
...
[Event Listeners](/docs/extensions/cross-chain-data/listeners.mdx) are responsible for observing external systems and storing information about the events that occur. When an event listener detects an event, it stores the event in its local database. The node will then broadcast information about the event to the rest of the network, until the event is recognized by a majority of the network.

[Resolutions](/docs/extensions/cross-chain-data/resolutions.mdx) are responsible for taking the information about the event from the event listener, and using it to trigger a state change in the network. When a resolution is triggered, it will broadcast information about the state change to the rest of the network, until the state change is recognized by a majority of the network.

## Event Lifecycle

The process by which the network agrees on external events can be broken down into the following steps:

1. Nodes observe an event in an external system, and broadcast this information to the rest of the network. The hash of the observed event, with the node it was observed by, is mined into a block.
...
When an event is observed by a node, the node will store the event in its local database. The node will immediately broadcast the ID of the event to the rest of the network.
