...
The oracle will read in the values set in `config.toml`, using them to connect to the Ethereum network and listen to the specified contract address.

## Step 4. Register the Event Listener

Register the oracle with the Kwil node. This is done by calling the `RegisterListener` function in the package's `init` function.
