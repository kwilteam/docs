...
subscription disconnections, Ethereum reorgs, and other edge cases. For a production-ready implementation of this event listener, please refer to the [Kwil EVM Deposits Listener](<https://github.com/kwilteam/kwil-db/tree/v0.7.0/extensions/listeners/eth_deposits>) that is
...
To build and run the event listener extension example, follow the standard [`kwild` build and run](/docs/extensions/building-kwild-with-extensions) process.
