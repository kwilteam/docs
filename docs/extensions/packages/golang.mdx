...
them with specific methods that they should provide, and an initializer function that creates a new instance of an extension. It's also possible to
...
It will also be called once for each Kuneiform schema that exists on a node when the node starts up.
