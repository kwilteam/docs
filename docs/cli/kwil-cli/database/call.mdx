...
Call a `view` procedure or action, returning the result.
...
```
kwil-cli database call <procedure_or_action> <parameter_1:value_1> <parameter_2:value_2> ... [flags]
```

### Examples

```
# Calling the `get_user($username)` procedure on the "mydb" database
